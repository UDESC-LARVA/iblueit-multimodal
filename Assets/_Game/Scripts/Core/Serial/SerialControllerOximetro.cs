 ...
 * class polls that queue by means of invoking SerialThreadOximetro.GetSerialMessage().
 *
 * The serial device must send its messages separated by a newline character.
 * Neither the SerialController nor the SerialThreadOximetro perform any validation
...
{
    public partial class SerialControllerOximetro : MonoBehaviour
    {
        ...
        private SerialThreadOximetro serialThread;
        ...
        private string AutoConnect()
        {
...
#if !UNITY_EDITOR
            if (ports.Length < 1)
                Ibit.Core.Util.SysMessage.Warning("OXÍMETRO não encontrado!");
#endif
            ...
            foreach (var port in ports)
            {
                ...
                {
                    //Debug.LogWarning($"Unable to connect {sp.PortName}:{sp.BaudRate}:OXÍMETRO.\n{e.GetType()}: {e.Message}");
                    sp.Close();
                    sp.Dispose();
                    continue;
                }
        ...
        public void Connect()
        {
            // Debug.Log("Looking for OXÍMETRO...");

            var portName = AutoConnect();

            if (string.IsNullOrEmpty(portName))
            {
                Debug.LogWarning("Failed to connect OXÍMETRO!");
                StartCoroutine(Reconnect());
                return;
            }

            serialThread = new SerialThreadOximetro(portName, baudRate, reconnectionDelay, maxUnreadMessages);
            ...
            Debug.Log($"Connected to {portName}:{baudRate}:OXÍMETRO");
        }
        ...
        private IEnumerator Reconnect()
        {
            ...
            //Debug.Log("Tentando reconectar Oxímetro...");
            Connect();
        }
        ...
        private void Disconnect()
        {
            ...
            Debug.Log("Serial disconnected:OXÍMETRO");
        }
