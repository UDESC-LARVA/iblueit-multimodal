 ...
 * class polls that queue by means of invoking SerialThreadMano.GetSerialMessage().
 *
 * The serial device must send its messages separated by a newline character.
 * Neither the SerialController nor the SerialThreadMano perform any validation
...
{
    public partial class SerialControllerMano : MonoBehaviour
    {
        ...
        private SerialThreadMano serialThread;
        ...
        private string AutoConnect()
        {
...
#if !UNITY_EDITOR
            if (ports.Length < 1)
                Ibit.Core.Util.SysMessage.Warning("MANOVACUÔMETRO not found!");
#endif
            ...
            foreach (var port in ports)
            {
                ...
                {
                    Debug.LogWarning($"Unable to connect {sp.PortName}:{sp.BaudRate}:MANOVACUÔMETRO.\n{e.GetType()}: {e.Message}");
                    sp.Close();
                    sp.Dispose();
                    continue;
                }
