...
{
    public class OximetroFlowMath
    {
        /// <summary>
        /// Calculates a mean of respiratory duration in Seconds per Cycle.
        /// </summary>
        /// <param name="data">Dictionary containing respiratory samples as [time,value] from OXÍMETRO.</param>
        ...
        public static float RespiratoryRate(Dictionary<float, float> data, int duration)
        {
            ...
            //     if (actualValue < -Pacient.Loaded.OximetroThreshold || actualValue > Pacient.Loaded.OximetroThreshold)
            //     {
            //         if (startTime == 0)
            //         {
            //             startTime = lastTime;
        ...
        private static float Poiseulle(float differentialPressure) =>
            differentialPressure * Mathf.PI * Mathf.Pow(Oximetro.Radius, 4) / (8 * Oximetro.AirViscosity * Oximetro.Lenght);
